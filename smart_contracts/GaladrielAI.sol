...
contract OpenAiSimpleLLM {
    ...
    function sendMessage(string memory _message) public {
        IOracle.Message memory tempMessage = createTextMessage(
            "user",
            _message
        );

        // Copy each element of the array manually
        delete message.content; // Clear existing storage content
        message.role = tempMessage.role;
        for (uint256 i = 0; i < tempMessage.content.length; i++) {
            message.content.push(tempMessage.content[i]);
        }
    ...
    // required for Oracle
    function getMessageHistory(uint256 _runId)
        public
        view
